...
export async function PATCH(request: Request, { params }: Params) {
  ...
  let response = await admin.supabase.from("vacancies").update(payload).eq("id", params.id).select("*").single();

  if (response.error && /column/i.test(response.error.message)) {
    response = await admin.supabase.from("vacancies").update(payloadBase).eq("id", params.id).select("*").single();
  }
...
export async function DELETE(request: Request, { params }: Params) {
  ...
  const { error } = await admin.supabase.from("vacancies").delete().eq("id", params.id);
