...
export default async function ResultPage({ params }: ResultPageProps) {
  ...
  const { data: attempt, error } = await supabase
    .from("attempts")
    .select("id, submitted_at, status, evidence_uploads(path, mime_type, size, created_at)")
    .eq("id", attemptId)
      ...
      <div className="mx-auto flex max-w-4xl flex-col gap-8 px-6 py-12">
        ...
        <Card className="border-slate-100 shadow-md">
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle className="text-xl text-slate-900">Proceso completado</CardTitle>
              <CardDescription>Tu evaluación fue enviada correctamente.</CardDescription>
            </div>
          ...
          <CardContent className="space-y-3 text-slate-800">
            ...
            <div className="rounded-lg bg-slate-50 p-4">
              ...
              <p className="text-sm text-slate-700">Nuestro equipo revisará tu información y te contactará con el seguimiento del proceso.</p>
              <p className="text-xs text-slate-500">Tiempo estimado de revisión: 2–3 días hábiles.</p>
            </div>
